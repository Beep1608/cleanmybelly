...
From the **root directory** of the repository (not inside `/app/frontend`), run the following command to bootstrap Next.js in a clean subfolder:
```bash
npx create-next-app@latest app/frontend --typescript --tailwind --eslint --app --src-dir --import-alias "@/*"
```
...
After initialization, your `/app/frontend` folder structure should look like this:
```text
app/frontend/
├── src/
│   ├── app/
...
1. Open the `next.config.mjs` (or `next.config.js`) file in the `/app/frontend` directory.
...
npm run build
```
This command compiles your Next.js application and creates a new directory named **`out/`** in `/app/frontend`. This `out/` folder contains the static HTML pages, CSS files, and JS chunks ready to be uploaded to AWS S3.
...
The GitHub Actions workflow under `.github/workflows/deploy-frontend.yml` is configured to automatically build and deploy this Next.js app to S3 whenever changes are pushed to `main` inside the `/app/frontend` directory. 
