...
- **Implementación:**
  ...
  - Se genera un archivo `backend.tfbackend` (`bucket = "cleanmybelly-tfstate-v1-..."`) en cada módulo.
...
### 4. Estructura de Entornos bajo `environments/` (Raíz del Proyecto)
...
### 6. Diseño de la Herramienta `tools/env_sync.py` (Modelo Híbrido)
...
graph TD
    ...
    P3["Paso 3: Crear el directorio raíz environments/ (global, dev, prod) con sus .example"] --> P4
    P4["Paso 4: Desarrollar la herramienta tools/env_sync.py (mapping.py, analyzer.py, writer.py)"] --> P5
...
### Especificación de `tools/env_sync.py`:
...
## ❓ PARTE 3: PREGUNTAS Y DECISIONES PENDIENTES DE CONFIRMACIÓN
...
1. **¿Confirmas que arrancamos de inmediato con el Paso 1 y Paso 2?**
   - Paso 1: Mover los bloques de `main.tf` a `providers.tf` en `aws/pre-infra/bootstrap` y eliminar los defaults `"JoseLopezLara"` y `"Test2"`.
   - Paso 2: Limpiar la línea `bucket = "<TERRAFORM_STATE_BUCKET_NAME>"` de todos los `providers.tf`.

2. **Detalle de Autodeclaración en `variables.tf` (Opcional):**
